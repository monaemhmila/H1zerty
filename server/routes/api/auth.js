...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!user) {
      console.log('Validation error: No user found for this email address');
      return res.status(400).send({ error: 'No user found for this email address.' });
    }

    if (user && user.provider !== EMAIL_PROVIDER.Email) {
      console.log(`Validation error: Email address is already in use using ${user.provider} provider`);
    ...
    if (!isMatch) {
      console.log('Validation error: Password Incorrect');
